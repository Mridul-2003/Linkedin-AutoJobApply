...
if len(sys.argv) > 1:
    try:
        user_config_json = sys.argv[1]
        user_config = json.loads(user_config_json)
        print("Configuration loaded from command-line argument.")
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON configuration: {e}")
        sys.exit(1)
else:
    print("Error: No JSON configuration provided as a command-line argument.")
    sys.exit(1)
...
required_keys = {
    ...
    "search_terms","resume_path" # Added resume_data here
}
...
current_city = user_config.get("current_city", "")
street = user_config.get("street", "")
state = user_config.get("state", "")
zipcode = user_config.get("zipcode", "")
country = user_config.get("country", "")
ethnicity = user_config.get("ethnicity", "")
gender = user_config.get("gender", "")
disability_status = user_config.get("disability_status", "")
veteran_status = user_config.get("veteran_status", "")
resume_data = user_config.get("resume_data")
# from search.py
search_terms = user_config.get("search_terms", [])
search_location = user_config.get("search_location", "")
sort_by = user_config.get("sort_by", "Most relevant")
date_posted = user_config.get("date_posted", "Any time")
experience_level = user_config.get("experience_level", [])
companies = user_config.get("companies", [])
job_type = user_config.get("job_type", [])
on_site = user_config.get("on_site", [])
easy_apply_only = user_config.get("easy_apply_only", False)
location = user_config.get("location", [])
industry = user_config.get("industry", [])
job_function = user_config.get("job_function", [])
job_titles = user_config.get("job_titles", [])
under_10_applicants = user_config.get("under_10_applicants", False)
in_your_network = user_config.get("in_your_network", False)
fair_chance_employer = user_config.get("fair_chance_employer", False)
salary = user_config.get("salary", "Any")
...
current_ctc_lakhs = str(round(300000 / 100000, 2))
current_ctc_monthly = str(round(300000/12, 2))
current_ctc = str(current_ctc)

notice_period_months = str(30//30)
notice_period_weeks = str(30//7)
...
genai.configure(api_key="AIzaSyAKjQbErgbnknBRZDrp63vAU93CKtiKNz0")
...
model = genai.GenerativeModel(
  ...
  system_instruction="remember all this when asked question you will answer from this data.\n"
                     """Respond to experience-based questions with only the relevant integer if the answer is in
                     format Python: 1 than only gives 1 and index from the provided JSON.
                      For all other questions like Have you ever created a website or not? provide a detailed 3-line answer in professional way for job applying on the basis of provided json by mentioning skills and libraries and models used .\n """
                     "if it is a multi option question only give the index number of the answer",
)
resume_data_json = resume_data

updated_json = resume_data_json
print(updated_json)
  ...
  history=[
    {
      "role": "user",
      "parts": [
          "My resume is as follows: " + str(updated_json) # Providing resume as a string inside 'parts'
      ],
    },
    {
      "role": "model",
...
def bard_flash_response(question) -> str:
    ...
    'zero': '0'
      ...
      for word in words:
        ...
        if cleaned_word in help_dict:
            numeric_word = cleaned_word
            break


      res = str(help_dict.get(numeric_word, 0))
      if res == 'None':
          return ans
...
def login_LN(driver, username, password):
    """
...
def upload_resume(modal: WebElement, resume: str) -> tuple[bool, str]:
    """
    Uploads a resume file via a modal WebElement.

    Args:
        modal (WebElement): The modal WebElement containing the file input.
        resume (str): Path to the resume file.

    Returns:
        tuple[bool, str]: A tuple containing a success flag and the file name or error message.
    """
    try:
        # Wait for the file input element to be present and interactable
        file_input = WebDriverWait(modal, 10).until(
            EC.presence_of_element_located((By.NAME, "file"))
        )
        
        # Check if the input is interactable or use JavaScript to set the value
        try:
            file_input.send_keys(os.path.abspath(resume))
        except Exception as e:
            print("Input not interactable, attempting with JavaScript...")
            driver.execute_script("arguments[0].style.display = 'block';", file_input)
            file_input.send_keys(os.path.abspath(resume))

        print(f"Resume uploaded: {os.path.basename(resume)}")
...
def answer_questions(modal: WebElement, questions_list: set, work_location: str) -> set:
    ...
    for Question in all_questions:
        ...
        text = Question.find_element(By.CSS_SELECTOR, "input.artdeco-text-input--input[type='text']")
        
        if text: 
            print("Text: ",text)
            do_actions = False
            label = Question.find_element(By.CLASS_NAME, "artdeco-text-input--label")
            print("Label: ",label)
            ...
            if not prev_answer or overwrite_previous_answers:
                # Use bard_flash_response to generate intelligent answers
                answer = bard_flash_response(f"What is my {label_org}?")
                
                # Fallback mechanisms if AI response is empty or unhelpful
                ...
                if do_actions:
                    WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "some_selector_for_the_element"))).send_keys(Keys.ARROW_DOWN)
                    actions.send_keys(Keys.ENTER).perform()
                    actions.send_keys(Keys.ENTER).perform()
...
#         driver.get(f"https://www.linkedin.com/jobs/search/?keywords={searchTerm}&f_AL=true")
...
#                                     try:
#                                         next_button = WebDriverWait(driver,3).until(EC.presence_of_element_located((By.XPATH, ".//span[text()='Next']"))) # Replace with correct CSS selector
#                                         next_button.click()
#                                         sleep(randint(2, 5)) 
#                                     except (NoSuchElementException, ElementClickInterceptedException) as e:
#                                         print_lg(f"Error clicking 'Next' button: {e}. Taking a screenshot and skipping...")
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    uploaded = False
                    # Case 1: Easy Apply Button
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3')]"):
                        print("Easy Apply")
                        try: 
                            try:
                                errored = ""
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        ...
                                        raise Exception("Seems like stuck in a continuous loop of next, probably because of new questions.")
                                    questions_list = answer_questions(modal, questions_list, work_location)
                                    if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, "/Volumes/PortableSSD/AutoJobsApply/resume (17).pdf")
...
def main() -> None:
    try:
        ...
        tabs_count = len(driver.window_handles)
        driver.get("https://www.linkedin.com/login")
        if not is_logged_in_LN(): login_LN(driver,username,password)
